...
use mprove::Errors::{self, MProveError};
use mprove::mprove_sigs::*;
...
#[derive(Clone, Debug)]
pub struct MProve {
    C_vec: Vec<EdwardsPoint>,
    P_vec: Vec<EdwardsPoint>,
    C_prime_vec: Vec<EdwardsPoint>,
    C_res: EdwardsPoint,
...
impl MProve{
    ...
    pub fn prove(
        ...
        let mut C_prime_vec: Vec<EdwardsPoint> = vec![*G; n];
        ...
        let message = EdwardsPoint::random(&mut rng);
        let z_vec: Vec<Scalar> = (0..n).map(|_| Scalar::random(&mut rng)).collect();

        let mut index: usize = 0;
        let C_res_vec: Vec<EdwardsPoint> = (0..n)
            .map(|i| {
        ...
        let C_res = EdwardsPoint::vartime_multiscalar_mul(one_vec.iter(), C_res_vec.iter());
    ...
    pub fn verify(
        ...
        // calculated C_res
        let C_sub_vec: Vec<EdwardsPoint> = (0..n)
            .map(|i| {
        ...
        let one_vec: Vec<Scalar> = (0..n).map(|_| Scalar::one()).collect();
        let C_res_comp = EdwardsPoint::vartime_multiscalar_mul(one_vec.iter(), C_sub_vec.iter());
    ...
    pub fn gen_params(n: usize, s: usize) -> (
        ...
        let mut P_vec: Vec<EdwardsPoint> = (0..n)
            .map(|_| {
        ...
        let mut C_vec_mut: Vec<EdwardsPoint> = (0..n)
            .map(|_| {
                EdwardsPoint::random(&mut rng)
            })
            .collect();
