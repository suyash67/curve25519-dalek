...
use mprove::Errors::{self, MProveSigsError};
...
impl RingSig{
    pub fn gen_RingSig(
        ...
        while j != index {
            ...
            // let L = sG + cpk_j;
            let L = EdwardsPoint::vartime_double_scalar_mul_basepoint(&c_old, &pk[j], &s_vec[j]);
    ...
    pub fn ver_RingSig(
        ...
        while j < n {
            ...
            // let cpk_j = pk[j] * c_old;
            // let L = sG + cpk_j;
            let L = EdwardsPoint::vartime_double_scalar_mul_basepoint(&c_old, &pk[j], &self.s_vec[j]);
...
impl LSAGSig{
    pub fn gen_LSAG(
        ...
        // compute key-image
        let H_P_idx = EdwardsPoint::hash_from_bytes::<Keccak512>(pk[index].compress().as_bytes());
        let I = H_P_idx * x;
        ...
        while j != index {
            ...
            let H_P = EdwardsPoint::hash_from_bytes::<Keccak512>(pk[j].compress().as_bytes());
        
            // compute R
            // let sH = H_P * s_vec[j];
            // let cj_I = I * c_old;
            // let R = sH + cj_I;
            let R = EdwardsPoint::vartime_multiscalar_mul(&[s_vec[j], c_old], &[H_P, I]);
    ...
    pub fn ver_LSAG(
        ...
        while j < n {
            ...
            // let R = sH + cj_I;
            let R = EdwardsPoint::vartime_multiscalar_mul(&[self.s_vec[j], c_old], &[H_P, self.I]);
    ...
    pub fn initialize(n: usize) -> LSAGSig {
        ...
        let s_vec = (0..n).map(|_| { Scalar::one() }).collect::<Vec<Scalar>>();
        let c = Scalar::one();
        let I = constants::ED25519_BASEPOINT_POINT;
...
mod tests {
    ...
    pub fn test_RingSig(n: usize, idx: usize){
        ...
        let G = constants::ED25519_BASEPOINT_POINT;
        let mut rng = rand::thread_rng();
        let msg = EdwardsPoint::random(&mut rng);

        let mut pk_vec: Vec<EdwardsPoint> = (0..n).map(|_| EdwardsPoint::random(&mut rng)).collect();
