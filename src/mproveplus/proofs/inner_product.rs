...
use mproveplus::errors::ProofError;
use mproveplus::transcript::TranscriptProtocol;
...
mod tests {
    ...
    fn test_helper_create(n: usize) {
        ...
        use mproveplus::generators::BulletproofGens;
        ...
        //             P = <a,G> + <b',H> + <a,b> Q,
        // where b' = b \circ y^(-n)
        let b_prime: Vec<Scalar> = b.iter().zip(H_factors.iter()).map(|(bi, yi)| bi * yi).collect();
        // a.iter() has Item=&Scalar, need Item=Scalar to chain with b_prime
        // let a_prime = a.iter().cloned();

        let P = EdwardsPoint::vartime_multiscalar_mul(
            a.iter().chain(b_prime.iter()).chain(iter::once(&c)),
