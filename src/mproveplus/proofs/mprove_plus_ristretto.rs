...
use mproveplus::Errors::{self, MProvePlusError};

use edwards::EdwardsPoint;
use traits::VartimeMultiscalarMul;
use scalar::Scalar;
use mproveplus::proofs::inner_product::InnerProductProof;
use mproveplus::util;
use std::vec::Vec;
use std::vec;
...
use rand::Rng;
use constants;
use mproveplus::generators::{PedersenGens};
...
impl MProvePlus {
    pub fn prove(
        // crs
        G: &EdwardsPoint,            // base for amount in a pedersen commitment
        H: &EdwardsPoint,            // base for blinding factor in a pedersen commitment
        Gt: &EdwardsPoint,           // instead of G1
        H_prime: &EdwardsPoint,      // base used in commitment to inner product
        p_vec: &[EdwardsPoint], 
        g_prime_vec: &[EdwardsPoint],
        h_vec: &[EdwardsPoint],
        g_vec_append: &[EdwardsPoint],
        h_vec_append: &[EdwardsPoint],
        // stmt
        C_vec: &[EdwardsPoint],      // vector of commitments
        P_vec: &[EdwardsPoint],      // addresses in the ring (public keys)
        H_vec: &[EdwardsPoint],      // hash of addresses
        // wit
        ...
        let mut I_vec_temp: Vec<EdwardsPoint> = vec![*G; s];
        let mut C_vec_temp: Vec<EdwardsPoint> = vec![*G; s];
        ...
        let C_res1 = EdwardsPoint::vartime_multiscalar_mul(one_vec.iter(), C_vec_temp.iter());
        let C_res = gamma_Gt + C_res1;

        // define compressed stmt Y_hat_vec
        let P_vec_temp: Vec<EdwardsPoint> = (0..n).map(|i| P_vec[i] * u).collect();
        ...
        let H_vec_temp: Vec<EdwardsPoint> = (0..n).map(|i| H_vec[i] * u_square).collect();
        // let H_vec_temp: Vec<RistrettoPoint> = (0..n).map(|i| 
        //     RistrettoPoint::vartime_multiscalar_mul(&[u_square.clone()], &[H_vec[i]])
        // ).collect();
        
        let Y_hat_vec: Vec<EdwardsPoint> = (0..n).map(|i| H_vec_temp[i] + P_vec_temp[i]).collect();
        ...
        let I_hat_vec: Vec<EdwardsPoint> = (0..s).map(|i| I_vec[i] * (minus_u_sq * v_s[i])).collect();
        ...
        // defining g_0
        let mut g_vec_0: Vec<EdwardsPoint> = Vec::with_capacity(t);
        ...
        let A = EdwardsPoint::vartime_multiscalar_mul(
        ...
        // defining g_w
        let mut g_vec_w: Vec<EdwardsPoint> = Vec::with_capacity(t);
        g_vec_w.extend_from_slice(&vec![*G, C_res, *Gt]);
        // let mut g_vec_w: Vec<EdwardsPoint> = vec![*G, C_res, *Gt];
        ...
        let Q = EdwardsPoint::hash_from_bytes::<Sha512>(b"test point");
    ...
    pub fn verify(
        ...
        // define compressed I_hat_vec
        let v_s: Vec<Scalar> = util::exp_iter(v).take(s).collect();
        let I_hat_vec: Vec<EdwardsPoint> = (0..s).map(|i| self.I_vec[i] * (minus_u_sq * v_s[i])).collect();
        // let I_hat_vec: Vec<RistrettoPoint> = (0..s).map(|i| 
        ...
        // let mut g_vec_w: Vec<EdwardsPoint> = vec![*G, self.C_res, *Gt];
        g_vec_w.extend_from_slice(&Y_hat_vec);
        g_vec_w.extend_from_slice(&C_vec);
        g_vec_w.extend_from_slice(&I_hat_vec);
    ...
    pub fn gen_params(n: usize, s: usize) -> (
        ...
        let H = EdwardsPoint::hash_from_bytes::<Sha512>(G.compress().as_bytes());
        let Gt = EdwardsPoint::hash_from_bytes::<Sha512>(b"block_height_100");
        let H_prime = EdwardsPoint::hash_from_bytes::<Sha512>(b"h_prime");
        ...
        let g_prime_vec: Vec<EdwardsPoint> = (0..g_prime_len).map(|_| EdwardsPoint::random(&mut rng)).collect();
        let h_vec: Vec<EdwardsPoint> = (0..h_len).map(|_| EdwardsPoint::random(&mut rng)).collect();

        let p_vec: Vec<EdwardsPoint> = (0..p_len).map(|_| EdwardsPoint::random(&mut rng)).collect();

        let g_vec_append: Vec<EdwardsPoint> = (0..res).map(|_| EdwardsPoint::random(&mut rng)).collect();
        let h_vec_append: Vec<EdwardsPoint> = (0..res).map(|_| EdwardsPoint::random(&mut rng)).collect();
        
        // generate random ring addresses and Hash of them 
        let mut P_vec: Vec<EdwardsPoint> = (0..n).map(|_| EdwardsPoint::random(&mut rng)).collect();
        let H_vec: Vec<EdwardsPoint> = (0..n).map(|_| EdwardsPoint::random(&mut rng)).collect();
        
        // Select random outputs owned by the exchange
        let mut C_vec_mut: Vec<EdwardsPoint> = (0..n).map(|_| EdwardsPoint::random(&mut rng)).collect();        
